...
class Calc_Emis_T(object):
    ...
    def calc_area(self):
        ...
        if self.grid_type == 'FV':
            ...
            self.grid_area = self.grid_area_rad2 / np.nansum( self.grid_area_rad2 ) \
                            * Earth_area
    # ============================ END Calculate area ========================
    # ========================================================================    
            ...
            if self.grid_type == 'FV':
                ...
                if 'time' in self.dimension:
                        ...
                        for ti in self.time_inds:
                            for vi, vt in enumerate(self.dim_var[self.vert_name]):
                                self.emissions_total_v[ti,vi] = \
                                    np.nansum(self.var_kg_m2_s[ti,vi, 
                            ...
                            self.emissions_total[ti] = np.nansum( self.emissions_total_v[ti,:] )

                    else:
                        for ti in self.time_inds:
                            self.emissions_total[ti] = \
                              np.nansum(self.var_kg_m2_s[ti,self.lat_inds[0]:self.lat_inds[-1]+1,
                                                         self.lon_inds[0]:self.lon_inds[-1]+1] \
                                     * self.grid_area[self.lat_inds[0]:self.lat_inds[-1]+1,
                                                      self.lon_inds[0]:self.lon_inds[-1]+1]) \
                    ...
                    if self.vert:
                        ...
                        for vi, vt in enumerate(self.dim_var[self.vert_name]):
                            self.emissions_total_v[vi] = np.nansum( \
                        ...
                        self.emissions_total = np.nansum( self.emissions_total_v )
                        
                    else:
                        self.emissions_total = np.nansum( \
                ...
                if 'time' in self.dimension:
                    ...
                    if self.vert:
                        ...
                        for ti in self.time_inds:
                            for vi, vt in enumerate(self.dim_var[self.vert_name]):
                                self.emissions_total_v[ti,vi] = \
                                      np.nansum(self.var_kg_m2_s[ti,vi,self.ncol_inds] \
                                                   * self.grid_area[self.ncol_inds]) \
                                                   * self.time_nseconds[ti] * self.vert_thick[vi]
                            self.emissions_total[ti] = np.nansum( self.emissions_total_v[ti,:] )
                    else:
                        for ti in self.time_inds:
                            self.emissions_total[ti] = np.nansum(self.var_kg_m2_s[ti,self.ncol_inds] \
                                                      * self.grid_area[self.ncol_inds]) \
                                                      * self.time_nseconds[ti]
                    ...
                    if self.vert:
                        self.emissions_total_v = np.zeros( len(self.dim_var[self.vert_name]) )
                        for vi, vt in enumerate(self.dim_var[self.vert_name]):
                            self.emissions_total_v[vi] = np.nansum(self.var_kg_m2_s[vi,self.ncol_inds] \
                                                      * self.grid_area[self.ncol_inds]) \
                                                      * self.ndays * 86400. * self.vert_thick[vi]
                        self.emissions_total = np.nansum( self.emissions_total_v )
                    else:
                        self.emissions_total = np.nansum(self.var_kg_m2_s[self.ncol_inds] \
                                              * self.grid_area[self.ncol_inds]) \
                                              * self.ndays * 86400.
            ...
            if self.grid_type == 'FV':
                if 'time' in self.dimension:
                    ...
                    if self.vert:
                        ...
                        for ti in self.time_inds:
                            for vi, vt in enumerate(self.dim_var[self.vert_name]):
                                self.emissions_total_v[ti,vi] = np.nansum(self.var_kg_m2_s[ti,vi,:,:] \
                                                     * self.grid_area) * self.time_nseconds[ti] \
                                                     * self.vert_thick[vi]
                            self.emissions_total[ti] = np.nansum( self.emissions_total_v[ti,:] )
                    else:
                        for ti in self.time_inds:
                            self.emissions_total[ti] = np.nansum(self.var_kg_m2_s[ti,:,:] \
                                                      * self.grid_area) * self.time_nseconds[ti]
                else:
                    if self.vert:
                        self.emissions_total_v = np.zeros( len(self.dim_var[self.vert_name]) )
                        for vi, vt in enumerate(self.dim_var[self.vert_name]):
                            self.emissions_total_v[vi] = np.nansum( self.var_kg_m2_s[vi,:,:] * \
                                                                 self.grid_area ) \
                                                     * self.ndays * 86400. * self.vert_thick[vi]
                        self.emissions_total = np.nansum( self.emissions_total_v )
                    else:
                        self.emissions_total = np.nansum( self.var_kg_m2_s * self.grid_area ) \
                                              * self.ndays * 86400.
