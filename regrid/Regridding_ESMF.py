    ...
    def __init__(self, var_array, fields=[], add_fields=[], dimension=[], dim_var={}, 
        ...
        if type(var_array) in [ xr.core.dataset.Dataset, xr.core.dataarray.DataArray ]:
            ...
            if type( var_array['time'].values[0] ) not in [cftime._cftime.DatetimeGregorian]:
                var_array['time'] = cftime.date2num(var_array['time'], 'days since 1750-01-01')
                var_array['time'].attrs['units'] = 'days since 1750-1-1 00:00:00'
                var_array['time'].attrs['calendar'] = '365_day'

            #         #print( np.dtype( var_array['time'].values[0] ).name )
            #         print( type(var_array['time'].values[0] ) )
            #         print( var_array['time'].values[0] )
            ...
            if type(var_array) in [ xr.core.dataset.Dataset ]:
                #self.var_array = var_array.fillna(0.)
