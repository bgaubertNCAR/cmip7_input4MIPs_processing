...
MODIFICATION HISTORY:
    ...
    Duseong Jo, 25, FEB, 2022: VERSION 4.20
    - Bug fix for species list provided with a list type
    Duseong Jo, 03, MAR, 2022: VERSION 4.30
    - Add allowed unit cases for aircraft emissions
    Duseong Jo, 25, MAR, 2022: VERSION 4.40
    - Bug fix for species with only specific sectors
    Duseong Jo, 23, JUN, 2022: VERSION 4.50
    - Add additional unit: from molecules to molecules
    Duseong Jo, 13, OCT, 2022: VERSION 4.60
    - Add a capability to deal with some manual option with just one file & species
    Duseong Jo, 10, APR, 2023: VERSION 4.70
    - Add more unit conversion option
'''
...
class sp_map(object):
    ...
    def __init__(self, mapping_file, nc_file_format='NETCDF3_64BIT_DATA', print_user_nl_cam=True, 
                 ...
                 ignore_warning=False, verbose=False, VOC_fix=False):
    ...
    def read_mapping_file(self):
        ...
        for ii, line in enumerate(self.file_all):
            if 'file locations end' in line.lower():
                ...
                Reading_Part2 = True
                continue
            elif 'mapping equations end' in line.lower():
                Reading_Part2 = False
                Reading_Part3 = True    
            ...
            if Reading_Part2:
                if Start_Reading_Part2:
                    ...
                    #print( self.Source_filelist ) 
                    #sys.exit()
                    # For one file & species case, we can't use get_sp
                    if len( self.Source_filelist ) == 1:
                        self.Source_sp = []
                        self.Source_files = self.Source_filelist[0]
                        self.one_file = True
                    else:
                        print( self.VOC_fix  )
                        self.Source_sp, self.Source_files = get_sp( self.Source_filelist, self.VOC_fix )
                        self.one_file = False

                    #print( get_sp( self.Source_filelist ) )
                    #print( self.Source_sp )
                    #print( type(self.Source_files) )

                    #if self.VOC_fix==True:
                    #    new_dict = {}
                    #    for i, (key, value) in enumerate(self.Source_files.items()):
                    #        key = "VOC" + key

                    #         print(key)
                    #         if i % 2 == 0:
                    #             new_dict[key] = "VOC" + value
                    #         else:
                    #             new_dict[key] =  value
                    #    self.Source_files = new_dict
                    #    self.Source_sp = ["VOC" + item for item in self.Source_sp]

                    #print( "BG", "self.VOC_fix==True")   
                ...
                for yi in yield_types_list:
                    ...
                    if yi in tmp:
                        ...
                        print( "BG", self.N_equations[sp_tmp] )
                        print( "BG", np.arange(self.N_equations[sp_tmp]) )
                        for ei in np.arange(self.N_equations[sp_tmp]):
                            #print(ei)
                            # For one file & species cas
                            print( self.Source_sp )
                            
                            if self.one_file & (self.Source_sp == []):
                                self.Source_sp.append( tmp[2+int(ei*2)].split('*')[-1] )
                                fntmp = self.Source_files
                                self.Source_files = { self.Source_sp[0]:fntmp }
                            
                            if tmp[2+int(ei*2)].split('*')[-1] in self.Source_sp:
                                self.eq_species[sp_tmp].append( tmp[2+int(ei*2)].split('*')[-1] )
                                pos_sp_border = tmp[2+int(ei*2)].rfind('*')
                                exec( 'self.eq_frac[sp_tmp].append( ' + tmp[2+int(ei*2)][:pos_sp_border] + ')' )
                                if ei > 0:
                                    self.eq_operators[sp_tmp].append( tmp[1+int(ei*2)] )

                            else:
                                raise ValueError( 'Check Source file format and species name!' + \
                                                  'Error in reading: ', tmp[2+int(ei*2)].split('*')[-1] )
    ...
    def remapping(self):
        ...
        self.nc_dims = {}
        if type(self.species_list) == list:
            self.species_to_process = self.species_list
        elif self.species_list.lower() == 'all':
            self.species_to_process = self.species_CESM
        else:
            raise ValueError( 'Check species list!' )
        ...
        for sp_cesm in self.species_to_process:
            ...
            if self.dimensions[sp_cesm] in ['vert','aircraft']:
                ...
                # calculate factor
                if self.Destination_unit in ['molecules cm-2 s-1', 'molecules/cm2/s', 'molecules/cm3/s', 'molecules/cm3/s']:
            ...
            if self.dimensions[sp_cesm] == 'sfc':
                ...
                if self.keep_sector:
                    ...
                    print( "BG Calculation first for 2d dimension" )
                    # Calculation first for 2d dimension
                    calc_str = 'self.var_value_2d = '
                    str_sector = ''
                    for sc in self.sectors[sp_cesm]:
    ...
    def convert_unit(self, sp_cesm, sp_src, sc):
        ...
        if self.Source_unit in ['kg m-2 s-1', 'kg/m2/s']:
        ...
        elif self.Source_unit in ['molecules cm-2 s-1', 'molecules/cm2/s']:
            ...
            if sp_cesm in self.diameter.keys():
                ...
                # unit: (particles/cm2/s)(molecules/mole)(g/kg)
                # MAM4 in CESM/CAM-chem has some weird unit for number
                self.conversion_factor[sp_cesm][sp_src] *= Avo * 1e3 / self.mass_per_particle[sp_cesm] \
                                                          * self.mw_CESM[sp_cesm] * 1e-3 / Avo
